...
class PracujScraper(BaseScraper):
    ...
    async def sort_offers_from_newest(self):
        ...
        await self.page.wait_for_timeout(500)
        try:
            await self.page.screenshot(path="reports/before_sort.png")
            await dropdown.wait_for(state="visible", timeout=30000)
            await dropdown.click()
            await self.page.screenshot(path="reports/after_sort_click.png")
        ...
        option = self.page.locator("[data-test=\"list-sort-type\"]").get_by_text("Najnowsze")

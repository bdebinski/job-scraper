...
class PracujOfferParser:
    ...
    async def parse(self) -> JobOffer:
        return JobOffer(
            ...
            requirements=clean_job_description(
                *await self.page.locator(
                    '[data-test="section-technologies"]'
                ).all_inner_texts()
            ),
            description=clean_job_description(
                await self.page.locator(self.locators.requirements).inner_text()
            ),
            url=self.page.url,
            status="TO_ANALYZE",
        )
...
class JustJoinItOfferParser:
    ...
    async def _get_requirements(self):
        requirements = await self.page.locator(
            self.locators.requirements
        ).all_inner_texts()
        return "\n".join(requirements)
    ...
    async def _parse_b2b_salary(self):
        try:
            return (
                await self.page.get_by_text("Net per month - B2B")
                .locator("..")
    ...
    async def _parse_perm_salary(self):
        try:
            return (
                await self.page.get_by_text("Gross per month")
                .locator("..")
                .first.inner_text(timeout=2000)
            )
        except playwright.async_api.TimeoutError:
